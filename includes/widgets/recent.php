...
<?php
   ...
   $transQuery = $db->query("SELECT * FROM trolley WHERE paid = 1 ORDER BY id DESC LIMIT 5");
   $results = array();
   while($row = mysqli_fetch_assoc($transQuery)){
   	$results[] = $row;
   }
  $row_count = $transQuery->num_rows;
  $used_ids = array();
...
<div id="recent_widget">
	<table class="table table-condensed">
	  ...
	  $productQuery = $db->query("SELECT id,title FROM products WHERE id = '{$id}'");
